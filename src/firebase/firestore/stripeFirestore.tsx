...
const getCheckoutSessionsCollectionRef = (
  userUid: string
...
export const subscribeToCheckoutSession = (
  ...
  const checkoutSessionDocRef = getCheckoutSessionDocRef(
    userUid,
    checkoutSessionId
  )
  ...
  return onSnapshot(
    ...
    (checkoutSessionSnap) => {
      if (checkoutSessionSnap.exists()) {
        getSessionCallback(
          checkoutSessionSnap.data() as Types.CheckoutSessionDocument
        )
      } else {
...
const getPaymentCollectionRef = (
  ...
  return collection(
    ...
    CUSTOMERS_FIRESTORE_COLLECTION,
    userUid,
    PAYMENTS_FIRESTORE_SUBCOLLECTION
  )
}
...
export const createPayment = (
  ...
  const checkoutSessionsCollectionRef =
    getCheckoutSessionsCollectionRef(userUid)

  addDoc(checkoutSessionsCollectionRef, {
    mode: 'payment',
    price: process.env.NEXT_PUBLIC_REACT_APP_stripePriceId,
