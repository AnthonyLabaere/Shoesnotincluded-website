...
export const subscribeToUser = (
  ...
  return onSnapshot(
    ...
    (userDocSnap) => {
      callback(
        userDocSnap.exists()
          ? (userDocSnap.data() as Types.UserDocument)
          : undefined
      )
    },
    (error) => {
...
export const subscribeToVoucherCardHistoryDocument = (
  userUid: string,
  callback: (
    userGameHistoryDocuments: Types.UserVoucherCardHistoryDocument[]
  ) => void
): (() => void) => {
  const voucherCardHistoryCollectionRef =
    userVoucherCardHistoryCollectionReference(userUid)
